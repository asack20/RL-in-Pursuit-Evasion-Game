...
class Robot:
    ...
    # vel = [linear, rotation]
    def move(self, vel):
        new_x = self.pose[0] + vel[0] * np.cos(self.pose[2]) * LIN_VEL_COEF
        new_y = self.pose[1] + vel[0] * np.sin(self.pose[2]) * LIN_VEL_COEF
        new_or = self.pose[2] + vel[1] * ANG_VEL_COEF

        if vel[1] != 0:
            new_x = new_x + vel[0] * np.cos(new_or) * LIN_VEL_COEF
            new_y = new_y + vel[0] * np.sin(new_or) * LIN_VEL_COEF
